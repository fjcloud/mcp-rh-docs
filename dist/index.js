...
function validateUrl(input) {
    ...
    const allowed = ALLOWED_HOSTS.some((h) => u.hostname === h || u.hostname.endsWith(`.${h}`));
    if (!allowed)
        throw new Error(`Host not allowed: ${u.hostname}. Allowed: ${ALLOWED_HOSTS.join(", ")}`);
    return u.href;
}
...
// ── Tool: search ────────────────────────────────────────────────────────────
async function searchDocs(query, site) {
    ...
    for (const m of text.matchAll(re)) {
        const url = m[2];
        if (url.includes("#:~:text=") || seenUrls.has(url))
            continue;
        seenUrls.add(url);
        const title = m[1]
...
server.registerTool("fetch_rh_doc", {
    ...
    try {
        return { content: [{ type: "text", text: await fetchDoc(url) }] };
    }
    catch (e) {
        return { content: [{ type: "text", text: `Error: ${e instanceof Error ? e.message : e}` }], isError: true };
    }
});
server.registerTool("search_rh_docs", {
    ...
    try {
        return { content: [{ type: "text", text: await searchDocs(query, site ?? "docs.redhat.com") }] };
    }
    catch (e) {
        return { content: [{ type: "text", text: `Error: ${e instanceof Error ? e.message : e}` }], isError: true };
    }
